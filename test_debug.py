...
def main():
    ...
    for name, dataloader in datamodule.test_dataloaders(test_set).items():
        ...
        confidence = 0
        label_length = 0
        init_dir(f'{debug_dir}/images/{name}')
        for imgs, labels, img_keys, img_origs in tqdm(iter(dataloader), desc=f'{name:>{max_width}}'):
            res = model.test_step((imgs.to(model.device), labels, img_keys, img_origs), -1, debug_dir, name)['output']
